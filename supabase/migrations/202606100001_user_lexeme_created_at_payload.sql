-- Include user_lexeme.created_at in vocabulary_changes payload for mobile stats sync.
...
declare
  ...
  v_lexeme_id uuid;
  v_lexeme_created_at timestamptz;
  v_snapshot_id uuid;
  v_revision bigint;
  v_language text;
...
begin
  ...
  if v_schema_version not in ('lexi.result.v1', 'lexi.jp-word-candidates.v1') then
    raise exception 'unsupported schemaVersion: %', v_schema_version;
  end if;

  if v_schema_version = 'lexi.result.v1' then
    v_part_of_speech := v_content->'translations'->0->>'note';
  else
    v_part_of_speech := null;
  end if;
  ...
  into v_lexeme_created_at
  from public.user_lexemes ul
  where ul.id = v_lexeme_id;
  ...
  if v_schema_version = 'lexi.result.v1'
    and jsonb_typeof(v_content->'inflections') = 'array' then
    for v_form in select * from jsonb_array_elements(v_content->'inflections')
    loop
      v_form_text := nullif(trim(v_form->>'form'), '');
      if v_form_text is null then
        continue;
      end if;

      v_form_key := lower(trim(v_form_text));
      v_relation := 'irregular';
      v_source := 'provider';
